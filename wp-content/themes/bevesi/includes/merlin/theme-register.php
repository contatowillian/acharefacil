...
function bevesi_get_registered_purchase_code() {
	return get_option( 'envato_purchase_code_52295978' );
}

add_action( 'admin_menu', 'bevesi_register_theme' );
function bevesi_register_theme() {
	add_menu_page( esc_html__( 'KlbTheme', 'bevesi' ), esc_html__( 'KlbTheme', 'bevesi' ), 'manage_options', 'klbtheme_options', 'bevesi_register_theme_options', 'dashicons-welcome-widgets-menus', 2);
	add_submenu_page( 'klbtheme_options', esc_html__('Register Theme', 'bevesi'), esc_html__('Register Theme', 'bevesi'), 'manage_options', 'klbtheme_options', 'bevesi_register_theme_options' );
	add_submenu_page( 'klbtheme_options', esc_html__('Theme Options', 'bevesi'), esc_html__('Theme Options', 'bevesi'), 'manage_options', 'customize.php?return=admin.php?page=klbtheme_options' );
}

function bevesi_register_theme_options() {
	...
	<?php if(!empty(bevesi_get_registered_purchase_code())){ ?>
		<div class="klb-help-container registered-container">
			...
			<div class="registered-right"> 
				<h2><?php esc_html_e('The theme registered','bevesi'); ?></h2>
				<p><?php esc_html_e('Envato allows 1 license for 1 project located on 1 domain. It means that 1 purchase key can be used only for 1 site. Each additional site will require an additional purchase key.','bevesi'); ?></p>
			</div>
		</div>
	<?php } ?>
	...
	<div class="klb-help-container">
		<div class="klb-help-item">
			<a href="https://klbtheme.com/doc/bevesi" target="_blank">
				...
				<h2><?php esc_html_e('Documentation', 'bevesi'); ?></h2>
			</a>
		</div>
		...
		<div class="klb-help-item">
			<a href="https://themeforest.net/user/klbtheme#contact" target="_blank">
				...
				<h2><?php esc_html_e('Reset License', 'bevesi'); ?></h2>
			</a>
		</div>
		...
		<div class="klb-help-item">
			<a href="https://klbtheme.ticksy.com/" target="_blank">
				...
				<h2><?php esc_html_e('Support', 'bevesi'); ?></h2>
			</a>
		</div>
		...
		<div class="klb-help-item">
			<a href="https://klbtheme.com/hire-us/" target="_blank">
				...
				<h2><?php esc_html_e('Hire Us', 'bevesi'); ?></h2>
			</a>
		</div>
	</div>
	...
	if(empty(bevesi_get_registered_purchase_code())){
		echo '<form action="" method="post" id="purchase_code_form">';
		echo '<h1>'.esc_html__('Register Theme','bevesi').'</h1>';
		echo '<p style="max-width: 500px;">'.esc_html__('You\'re almost done. Just one more step. In order to gain full access to all demos, 
		premium plugins and support, please register your theme\'s purchase code.','bevesi').'</p>';
		echo '<h2>'.esc_html__('Your Envato Purchase Code','bevesi').'</h2>';
		
		echo '<p>';
		echo '<input class="regular-text code" type="text" name="purchase_code" id="purchase_code" value=""> ';
		echo '<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">'.esc_html__('Where to find the code?','bevesi').'</a>';
		echo '</p>';
		
		echo '<p>';
		echo '<label>';
		echo '<input type="checkbox" id="klb-accept-license-terms">'.esc_html__('I confirm that, according to the Envato License Terms, I am licensed to use the purchase code for a single project. Using it on multiple installations is a copyright violation.','bevesi').'</label>';
		echo '<a href="https://themeforest.net/licenses/terms/regular" target="_blank">'.esc_html__('License details.','bevesi').'</a>';
		echo '</p>';
		
		echo '<p class="klb-actions">';
		echo '<button class="button button-primary button-hero" id="klb-register-theme" disabled="disabled">'.esc_html__('Register Theme','bevesi').'</button>';
		echo '</p>';
		...
		<div class="c-inner klbtheme-status">
			<h2><?php esc_html_e( 'WordPress', 'bevesi' ); ?></h2>
			<div class="klb-table klb-odd">
				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'Theme Name', 'bevesi' ); ?>:
					</div>
					<div>
				...
				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'Theme Version', 'bevesi' ); ?>:
					</div>
					<div>
				...
				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'WP Version', 'bevesi' ); ?>:
					</div>
					<div>
				...
				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'WP Multisite', 'bevesi' ); ?>:
					</div>
					<div>
						<?php echo is_multisite() ? esc_html__( 'Yes', 'bevesi' ) : esc_html__( 'No', 'bevesi' ); ?>
					</div>
				</div>

				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'WP Debug Mode', 'bevesi' ); ?>:
					</div>
					<div>
						<?php echo defined( 'WP_DEBUG' ) && WP_DEBUG ? esc_html__( 'Enabled', 'bevesi' ) : esc_html__( 'Disabled', 'bevesi' ); ?>
					</div>
				</div>
			</div>
			<h2><?php esc_html_e( 'Server', 'bevesi' ); ?></h2>
			<div class="klb-table klb-odd">

				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'PHP Version', 'bevesi' ); ?>:
					</div>
					<div>
						<?php if ( version_compare( PHP_VERSION, '7.2', '<' ) ) : ?>
							<div class="klb-status-error">
								...
								<span>
									<?php esc_html_e( 'Minimum required PHP version 7.2', 'bevesi' ); ?>
								</span>
							</div>
						<?php else : ?>
							<?php echo esc_html( PHP_VERSION ); ?>
				...
				<?php if ( function_exists( 'ini_get' ) ) : ?>
					<div class="klb-table-row">
						<div>
							...
							<?php esc_html_e( 'PHP Post Max Size', 'bevesi' ); ?>:
						</div>
						...
						<div>
							<?php if ( wp_convert_hr_to_bytes( $post_max_size ) < 64000000 ) : ?>
								<div class="klb-status-error">
									...
									<span>
										<?php esc_html_e( 'Minimum required value 64M.', 'bevesi' ); ?>
									</span>
								</div>
							<?php else : ?>
								<?php echo esc_html( $post_max_size ); ?>
					...
					<div class="klb-table-row">
						<div>
							...
							<?php esc_html_e( 'PHP Time Limit', 'bevesi' ); ?>:
						</div>
						...
						<div>
							<?php if ( $max_execution_time < 180 ) : ?>
								<div class="klb-status-error">
									...
									<span>
										<?php esc_html_e( 'Minimum required value 180.', 'bevesi' ); ?>
									</span>
								</div>
							<?php else : ?>
								<?php echo esc_html( $max_execution_time ); ?>
					...
					<div class="klb-table-row">
						<div>
							...
							<?php esc_html_e( 'PHP Max Input Vars', 'bevesi' ); ?>:
						</div>
						...
						<div>
							<?php if ( $max_input_vars < 10000 ) : ?>
								<div class="klb-status-error">
									...
									<span>
										<?php esc_html_e( 'Minimum required value 10000.', 'bevesi' ); ?>
									</span>
								</div>
							<?php else : ?>
								<?php echo esc_html( $max_input_vars ); ?>
					...
					<div class="klb-table-row">
						<div>
							...
							<?php esc_html_e( 'PHP Memory Limit', 'bevesi' ); ?>:
						</div>
						...
						<div>
							<?php if ( wp_convert_hr_to_bytes( $memory_limit ) < 128000000 ) : ?>
								<div class="klb-status-error">
									...
									<span>
										<?php esc_html_e( 'Minimum required value 128M.', 'bevesi' ); ?>
									</span>
								</div>
							<?php else : ?>
								<?php echo esc_html( $memory_limit ); ?>
					...
					<div class="klb-table-row">
						<div>
							...
							<?php esc_html_e( 'PHP Upload Max Size', 'bevesi' ); ?>:
						</div>
						<div>
					...
					<div class="klb-table-row">
						<div>
							<?php esc_html_e( 'PHP Function "file_get_content"', 'bevesi' ); ?>:
						</div>
						<div>
							<?php if ( ! ini_get( 'allow_url_fopen' ) || 'Off' === ini_get( 'allow_url_fopen' ) ) : ?>
								<div class="klb-status-error">
									<?php esc_html_e( 'Off', 'bevesi' ); ?>
								</div>
							<?php else : ?>
								<?php esc_html_e( 'On', 'bevesi' ); ?>
							<?php endif; ?>
						</div>
					</div>
				<?php endif; ?>
				...
				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'DOMDocument', 'bevesi' ); ?>:
					</div>
					<div>
						<?php if ( ! class_exists( 'DOMDocument' ) ) : ?>
							<div class="klb-status-error">
								<?php esc_html_e( 'No', 'bevesi' ); ?>
							</div>
						<?php else : ?>
							<?php esc_html_e( 'Yes', 'bevesi' ); ?>
						<?php endif; ?>
					</div>
				</div>
				...
				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'Active Plugins', 'bevesi' ); ?>:
					</div>
					<div>
						<?php if ( is_multisite() ) : ?>
	...
	if(isset($_POST['purchase_code']) && wp_verify_nonce( $_POST['register_theme_nonce_field'], 'register_theme_my_action' )) {
		...
		echo '<div class="data-response success">'.esc_html__('The theme registered succesfully','bevesi').'</div>';
		update_option( 'envato_purchase_code_52295978', $purchase_code );
	}
}


function bevesi_is_theme_registered() {
	$purchase_code =  bevesi_get_registered_purchase_code();
...
$bevesi_tgmpa_prefix = ( defined( 'WP_NETWORK_ADMIN' ) && WP_NETWORK_ADMIN ) ? 'network_admin_' : '';
add_filter( 'tgmpa_' . $bevesi_tgmpa_prefix . 'plugin_action_links', 'bevesi_tgmpa_filter_action_links', 10, 4 );
function bevesi_tgmpa_filter_action_links( $action_links, $item_slug, $item, $view_context ) {
	$source = ! empty( $item['source'] ) ? $item['source'] : '';

	// Prevent installing theme's premium plugins.
	if ( 'External Source' === $source && ! bevesi_is_theme_registered() ) {
		$action_links = array(
			'bevesi_registration_required' => sprintf( __( '<a style="color: #ff0000;" href="%s">Register theme to unblock it</a>', 'bevesi' ), esc_url( admin_url( 'themes.php?page=register-theme' ) ) ),
		);
	}
 ...
 * Admin Notice
 */
add_action('admin_notices', 'bevesi_notice_for_activation');
function bevesi_notice_for_activation() {

	if(empty(bevesi_get_registered_purchase_code())){

		echo '<div class="notice notice-warning">
			<p>' . sprintf(
			esc_html__( 'Enter your Envato Purchase Code to receive Theme and plugin updates %s', 'bevesi' ),
			'<a href="' . admin_url('admin.php?page=klbtheme_options') . '">' . esc_html__( 'Enter Purchase Code', 'bevesi' ) . '</a>') . '</p>
		</div>';
	}
}