...
/*----------------------------
  Product Type 2
 ----------------------------*/
function bevesi_product_type2($stockprogressbar = '', $stockstatus = '', $shippingclass = '', $countdown = ''){
	global $product;
	global $post;
	global $woocommerce;
	...
	$gallery = get_theme_mod('bevesi_product_hover_gallery') == 1 ? 'product-thumbnail' : '';
		...
		$output .= '<h2 class="product-title"><a href="'.get_permalink().'"><span>'.get_the_title().'</span><br><br><span style="font-weight:300">'.get_the_content().'</span></a></h2>';
		...
		$output .= '<div class="product-content-fade style-1"></div>';
