...
define('TABLEON_VERSION', '1.0.4.2');
...
class TABLEON {
    ...
    public function draw_table_data($args, $table_html_id, $as_script = 'tableon-table-json-data') {
        ...
        $js_script_data = [
			...
			'wp_nonce' => wp_create_nonce('tableon'),
            'ajax_url' => admin_url('admin-ajax.php'),
            'selected_lang' => apply_filters('tableon_current_lang', get_locale()),
    ...
    public function get_table_data($return = false, $request_data = []) {
        ...
        if (isset($request_data['shortcode_args_set'])) {
            $shortcode_args_set = TABLEON_HELPER::sanitize_array(unserialize((stripslashes($request_data['shortcode_args_set']))));
    ...
    public function get_smth() {
        ...
        switch ($what) {
            ...
            case 'shortcodes_set':
                ...
                $content = TABLEON_HELPER::delete_not_allowed_shortcodes(esc_html($_REQUEST['shortcodes_set']));
                $res = TABLEON_HELPER::sanitize_array(do_shortcode($content));
                break;
...
add_shortcode('tableon_popup_iframe_button', function ($args) {
    ...
    if (isset($args['popup_page_link'])) {
        $popup_page_link = sanitize_url($args['popup_page_link']);
    }
    ...
    $popup_title = '';
    if (isset($args['popup_title'])) {
        $popup_title = esc_html($args['popup_title']);
    }

    $css_class = '';
    if (isset($args['css_class'])) {
        $css_class = esc_attr($args['css_class']);
    }

    $help_title = '';
    if (isset($args['help_title'])) {
        $help_title = TABLEON_Vocabulary::get(esc_html($args['help_title']));
        $help_title = trim($help_title, '"');
        $help_title = trim($help_title, "'");
        unset($args['help_title']);
    }
    ...
    $help_link = '';
    if (isset($args['help_link'])) {
        $help_link = sanitize_url($args['help_link']);
        unset($args['help_link']);
    }
