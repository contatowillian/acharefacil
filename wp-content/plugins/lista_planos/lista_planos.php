...
function inseri_dados_pagamento($dados) {
  global $wpdb;
   ...
   $select_ultimo_pagamento_id_user = "select id_pagamento from `pagamento` where pagamento_efetuado is null and tipo_plano= '".$dados['tipo_plano']."'  and user_id  = ".$dados['user_id'];
...
function content_mostraListasPlanos($content) {
    ...
    if (is_page('cadastro_sucesso')) {
      ...
      if ( is_user_logged_in() ) {
       ...
       $botao_compartilhar = '<a style="background-color: #336838" class="elementor-button elementor-button-link elementor-size-sm" href="https://acharefacil.com.br/detalhe/?detalhe_anunciante='.$id_usuario_logado.'" id="ver-meu-anuncio-botao">
                                <span class="elementor-button-content-wrapper">
                                      <span class="elementor-button-text">Ver meu anúncio</span>
                                </span>
                              </a>';
