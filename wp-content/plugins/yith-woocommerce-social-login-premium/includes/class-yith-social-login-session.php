...
if ( ! class_exists( 'YITH_WC_Social_Login_Session' ) ) {
	...
	class YITH_WC_Social_Login_Session {
		...
		public function set_login_cookie( $set = true ) {
			if ( ! headers_sent() ) {
				if ( $set ) {
					@setcookie( 'ywsl_login_process', '1', time() + 30 * 60, COOKIEPATH, COOKIE_DOMAIN, false ); //phpcs:ignore
				} else {
					if ( isset( $_COOKIE['ywsl_login_process'] ) ) {
						@setcookie( 'ywsl_login_process', '', time() - 3600, COOKIEPATH, COOKIE_DOMAIN, false ); //phpcs:ignore
					}
				}
			}
		}
		...
		public function get_blacklist() {
			$blacklist = apply_filters(
				'ywsl_session_start_uri_blacklist',
				array(
					'feed',
					'feed/rss',
					'feed/rss2',
	 ...
	 * @return \YITH_WC_Social_Login_Session
	 */
	function YITH_WC_Social_Login_Session() { //phpcs:ignore
