...
if ( ! class_exists( 'YITH_WC_Social_Login_Premium' ) ) {
	...
	class YITH_WC_Social_Login_Premium extends YITH_WC_Social_Login {
			...
			if ( ! is_admin() ) {
				if ( get_option( 'ywsl_show_in_checkout' ) === 'yes' ) {
					...
					add_action(
						...
						array( YITH_WC_Social_Login_Frontend(), 'social_buttons_in_checkout_blocks' ),
						10
					);
				}
		 ...
		 * @return array|string
		 */
		public function get_social_login_connection( $user_id, $image_size = 30, $output = 'buttons' ) {
			global $wpdb;
			$query        = $wpdb->prepare( "SELECT meta_key from $wpdb->usermeta WHERE meta_key LIKE '%%_login_id' AND user_id=%d", $user_id ); //phpcs:ignore
			$connections  = $wpdb->get_results( $query, ARRAY_A ); //phpcs:ignore
			$buttons      = '';
			$list         = array();
		 ...
		 * @return false|mixed
		 */
		public function get_user_info( $userid, $info, $provider ) {
		 ...
		 * @since    1.0.0
		 */
		public function social_unlink() {
			if ( ( defined( 'DOING_AJAX' ) && DOING_AJAX ) || ! isset( $_REQUEST['nonce_social'] ) || ! isset( $_REQUEST['provider'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce_social'] ) ), 'unlink-social-' . sanitize_text_field( wp_unslash( $_REQUEST['provider'] ) ) ) ) { //phpcs:ignore
				return;
			}
		...
		public function register_plugin_for_activation() {
			if ( ! class_exists( 'YIT_Plugin_Licence' ) ) {
				require_once YITH_YWSL_DIR . 'plugin-fw/lib/yit-plugin-licence.php';
			}
	 ...
	 * @return \YITH_WC_Social_Login_Premium
	 */
	function YITH_WC_Social_Login_Premium() { //phpcs:ignore
