...
if ( ! function_exists( 'yith_ywsl_install_woocommerce_admin_notice' ) ) {
	/**
	 * Required WooCommerce Message
	 */
	function yith_ywsl_install_woocommerce_admin_notice() { ?>
		<div class="error">
			<p><?php esc_html_e( 'YITH Woocommerce Social Login is enabled but not effective. It requires WooCommerce in order to work.', 'yith-woocommerce-social-login' ); ?></p>
		</div>
		<?php
	}
}
...
if ( defined( 'YITH_YWSL_VERSION' ) ) {
	...
	define( 'YITH_YWSL_VERSION', '1.38.0' );
}
...
if ( ! function_exists( 'yith_ywsl_premium_install' ) ) {
	...
	function yith_ywsl_premium_install() {
		...
		if ( ! function_exists( 'WC' ) ) {
			...
			add_action( 'before_woocommerce_init', 'ywsl_add_support_hpos_system' );
...
if ( ! function_exists( 'ywsl_add_support_hpos_system' ) ) {
    function ywsl_add_support_hpos_system() {
	    if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) ) {
		    \Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', YITH_YWSL_INIT );
	    }
    }
}
