...
class widget_search_filter extends WP_Widget { 
	...
	function __construct() {
		$widget_ops = array('description' => esc_html__('Display the search filter','bevesi-core') );
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'search_filter' );
		 parent::__construct( 'search_filter', esc_html__('Bevesi Search Filter','bevesi-core'), $widget_ops, $control_ops );
	}
			...
			<div class="widget-body"> 
				...
				<?php
					...
					if($attribute_items){
						...
						wp_enqueue_script('bevesi-attribute-filter');
						...
						foreach($attribute_array as $item_name){
							...
							echo '<select class="theme-select" name="filter_'.esc_attr($item_name).'" id="filter_'.esc_attr($item_name).'" tax="pa_'.$item_name.'" data-placeholder="'.esc_attr__('Select', 'bevesi-core').' '.esc_attr($label_name).'" data-search="true" data-searchplaceholder="'.esc_attr__('Search item...', 'bevesi-core').'">';
							...
							foreach ($terms as $term) {
								...
								if($term_children && $childcount == 1){
									echo '<div class="form-column">';
									echo '<select class="child-attr theme-select" id="child_filter_'.esc_attr($item_name).'" name="filter_'.esc_attr($item_name).'" data-placeholder="'.esc_attr__('Select Model', 'bevesi-core').'" data-search="true" data-searchplaceholder="'.esc_attr__('Search item...', 'bevesi-core').'" disabled>';
						...
						echo '<div class="form-column">';
						echo '<button class="btn primary">'.esc_html__('Find Auto Parts','bevesi-core').'</button>';
						echo '</div>';
