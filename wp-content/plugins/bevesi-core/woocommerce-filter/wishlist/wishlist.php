...
if ( ! class_exists( 'KlbWishlist' ) ) {
	class KlbWishlist {
		...
		function init() {
			...
			// add button for bevesi_wishlist_action
			add_action( 'bevesi_wishlist_action', [ $this, 'bevesi_wishlist_button' ], 11 );
			
			// add button for single
			add_action( 'bevesi_single_wishlist', [$this,'bevesi_wishlist_button'], 40);
		...
		function wishlist_add() {
			...
			if ( ( $product_id = (int) sanitize_text_field( $_POST['product_id'] ) ) > 0 ) {
				...
				$return['totalcount'] = sprintf(_n('%d person', '%d people', $totalcount, 'bevesi-core'), $totalcount);
				$return['data']   = [
					'key'       => self::get_key(),
					'ids'       => self::get_ids(),
				];
				...
				$product_id       = 0;
				$return['status'] = 0;
				$return['notice'] = esc_html__( 'Have an error, please try again!', 'bevesi-core' );
			}
		...
		function wishlist_remove() {
			...
			if ( ( $product_id = (int) sanitize_text_field( $_POST['product_id'] ) ) > 0 ) {
				...
				if ( array_key_exists( $product_id, $products ) ) {
					...
					$return['count']  = count( $products );
					$return['status'] = 1;
					$return['notice'] = esc_html__( 'Product has been removed from the Wishlist.', 'bevesi-core' );
					$return['data']   = [
						'key'       => self::get_key(),
						'ids'       => self::get_ids(),
					];
					...
					if ( empty( $products ) ) {
						$return['content'] = self::wishlist_content( $key, esc_html__( 'There are no products on the Wishlist!', 'bevesi-core' )) . '</div>';
					}
				} else {
					$return['notice'] = esc_html__( 'The product does not exist on the Wishlist!', 'bevesi-core' );
				}
			} else {
				$product_id       = 0;
				$return['notice'] = esc_html__( 'Have an error, please try again!', 'bevesi-core' );
		...
		function bevesi_wishlist_button() {
			...
			if($wishlist == '1'){
				...
				if ( $attr_id ) {
					...
					if ( array_key_exists( $attr_id, self::$products ) || in_array( $attr_id, array_column( self::$products, 'parent' ) ) ) {
						...
						$text  = apply_filters( 'klbwl_button_text_added', esc_html__( 'View wishlist', 'bevesi-core' ) );
					} else {
						$text = apply_filters( 'klbwl_button_text', esc_html__( 'Add to wishlist', 'bevesi-core' ) );
					}
					
					$output .= '<div class="wishlist-button product-button product-wishlist">';
					
					if (! is_admin() && is_product() ) {
						$output .= '<a href="'.esc_url( self::get_url() ).'" class="'.esc_attr( $class ).'" data-product_id="'.esc_attr( $attr_id ).'" data-product_title="'. esc_attr($product_name ).'"></a>';
		...
		function bevesi_klbwl_total_count(){
			global $product;
			$output;
		...
		function shortcode_list( $attrs ) {
			...
			$return_html .= '<div class="footer-social">';
 			$return_html .= '<span class="d-block text-12 mt-30 sm-mt-50 mb-10">'.esc_html__('Share on:', 'bevesi-core').'</span>';
			$return_html .= '<div class="site-social">';
			$return_html .= '<ul>';
			$return_html .= '<li><a href="https://www.facebook.com/sharer.php?u=' . $share_url . '" class=" md facebook"><i class="klb-social-icon-facebook"></i></a></li>';
			$return_html .= '<li><a href="https://twitter.com/share?url=' . $share_url . '" class=" md twitter"><i class="klb-social-icon-twitter"></i></a></li>';
			$return_html .= '<li><a href="https://instagram.com/pin/create/button/?url=' . $share_url . '" class=" md instagram"><i class="klb-social-icon-instagram"></i></a></li>';
			$return_html .= '<li><a href="https://linkedin.com/pin/create/button/?url=' . $share_url . '"" class=" md linkedin"><i class="klb-social-icon-linkedin"></i></a></li>';
			$return_html .= '</ul>';
		...
		function account_items( $items ) {
			if ( ! isset( $items['wishlist'] ) ) {
				$items['wishlist'] = apply_filters( 'klbwl_myaccount_wishlist_label', esc_html__( 'Wishlist', 'bevesi-core' ) );
			}
		...
		function enqueue_scripts() {
			...
			// main style
			wp_enqueue_style( 'bevesi-wishlist', plugins_url( 'css/wishlist.css', __FILE__ ), false, '1.0');

			// main js
			wp_enqueue_script( 'bevesi-wishlist',  plugins_url( 'js/wishlist.js', __FILE__ ),[
				'jquery',
				'js-cookie'
			], true );
			...
			// localize
			wp_localize_script( 'bevesi-wishlist', 'klbwl_vars', [
					'ajax_url'            => admin_url( 'admin-ajax.php' ),
					'wishlist_url'        => self::get_url(),
					'empty_confirm'       => esc_html__( 'This action cannot be undone. Are you sure?', 'bevesi-core' ),
					'delete_confirm'      => esc_html__( 'This action cannot be undone. Are you sure?', 'bevesi-core' ),
					'menu_text'           => apply_filters( 'klbwl_menu_item_label', esc_html__( 'Wishlist', 'bevesi-core' ) ),
					'button_text'         => apply_filters( 'klbwl_button_text', esc_html__( 'Add to wishlist', 'bevesi-core' ) ),
					'button_text_added'   => apply_filters( 'klbwl_button_text_added', esc_html__( 'View wishlist', 'bevesi-core' ) ),
				]
			);
		}
		...
		function get_items( $key, $layout = null ) {
			...
			if ( is_array( $products ) && ( count( $products ) > 0 ) ) {
				...
				echo '<thead>';
				echo '<tr>';
				echo '<th class="product-thumbnail"><span class="screen-reader-text">'.esc_html__( 'Thumbnail image', 'bevesi-core' ).'</span></th>';
				echo '<th class="product-name">'.esc_html__( 'Product', 'bevesi-core' ).'</th>';
				echo '<th class="product-price">'.esc_html__( 'Price', 'bevesi-core' ).'</th>';
				echo '<th class="product-date">'.esc_html__( 'Date Added', 'bevesi-core' ).'</th>';
				echo '<th class="product-subtotal">'.esc_html__( 'Stock', 'bevesi-core' ).'</th>';
				echo '<th class="product-addtocart">'.esc_html__( 'Add to cart', 'bevesi-core' ).'</th>';
				echo '<th class="product-remove"><span class="screen-reader-text">'.esc_html__( 'Remove item', 'bevesi-core' ).'</span></th>';
				echo '</tr>';
				echo '</thead>';
				echo '<tbody>';
				foreach ( $products as $product_id => $product_data ) {
					...
					echo '<span><i class="klb-icon-x"></i></span>';
					echo '</td>';
				...
				echo '<div class="klbwl-no-result">
						<div class="cart-empty-page">
							...
							<p class="cart-empty">'.esc_html__( 'The wishlist table is empty.', 'bevesi-core' ).'</p>
							<p class="return-to-shop">
								<a class="button wc-backward'.esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ).'" href="'.esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ).'">
									'.esc_html( apply_filters( 'woocommerce_return_to_shop_text', esc_html__( 'Return to shop', 'bevesi-core' ) ) ).'
								</a>
							</p>
						</div>
					</div>';
		...
		function wishlist_content( $key = false, $message = '' ) {
			...
			$name     = esc_html__( 'Wishlist', 'bevesi-core' );
			...
			<div class="klbwl-popup-inner" data-key="<?php echo esc_attr( $key ); ?>">
				<div class="klbwl-popup-content">
					...
					<a class="btn primary klbwl-page" href="<?php echo KlbWishlist::get_url(); ?>">
						<i class="klb-icon-hearth"></i> <?php echo esc_html__( 'View Wishlist', 'bevesi-core' ); ?>
					</a>
					<a class="btn primary klbwl-popup-close" href="#" data-url="#">
						<i class="klb-icon-x"></i> <?php echo esc_html__( 'Close', 'bevesi-core' ); ?>
					</a>
				</div>
			</div>
			<?php
		...
		public static function get_page_id() {
			if(get_theme_mod('bevesi_wishlist_page')){
				return absint(get_theme_mod('bevesi_wishlist_page'));
			}
		...
		function bevesi_delete_all_data(){
