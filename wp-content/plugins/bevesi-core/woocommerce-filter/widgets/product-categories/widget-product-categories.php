...
class klb_widget_klb_product_categories extends WP_Widget { 
	...
	function __construct() {
		$widget_ops = array('description' => esc_html__('For Main Shop Page.','bevesi-core') );
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'klb_product_categories' );
		 parent::__construct( 'klb_product_categories', esc_html__('Bevesi Product Categories','bevesi-core'), $widget_ops, $control_ops );
	}
	...
	function widget($args, $instance) {
		if(!is_product_category()){
			...
			foreach ( $terms as $term ) {
				...
				echo '<li>';
				echo '<a href="'.esc_url(bevesi_get_cat_url($term->term_id)).'" class="product_cat">';
					...
					if($term_children){
						...
						foreach($term_children as $child){
							...
							if($childterm->parent && (sizeof($term_third_children)>0)){
								echo '<li>';
								echo '<a href="'.esc_url( bevesi_get_cat_url($childterm->term_id)).'">';
								...
								if($term_third_children){
									...
									foreach($term_third_children as $third_child){
										...
										echo '<li>';
										echo '<a href="'.esc_url( bevesi_get_cat_url($thirdchildterm->term_id)).'">';
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('exclude'); ?>"><?php esc_html_e('Exclude id:','bevesi-core'); ?></label>
