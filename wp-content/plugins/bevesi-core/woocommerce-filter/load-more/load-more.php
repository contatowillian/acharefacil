...
if(get_theme_mod('bevesi_paginate_type') == 'loadmore' || bevesi_ft() == 'load-more'){
	function bevesi_load_more_button(){
		echo '<nav class="woocommerce-pagination klb-load-more">
                <ul class="page-numbers">
					<div class="btn btn-dark">'.esc_html__('Load More','bevesi-core').'</div>
				</ul>
			  </nav>';
	}

	remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
	add_action( 'woocommerce_after_shop_loop', 'bevesi_load_more_button', 10 );
}
...
if(get_theme_mod('bevesi_paginate_type') == 'infinite' || bevesi_ft() == 'infinite'){
	remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
}
...
if(get_theme_mod('bevesi_ajax_on_shop',0) == '1'){
	add_action('woocommerce_before_main_content','bevesi_loadmore_parameters',40);
	function bevesi_loadmore_parameters() {
	...
	<script type="text/javascript">
		var loadmore = {
			...
			"no_more_products":"<?php echo esc_html_e('No More Products','bevesi-core'); ?>",
...
## Quick View Scripts
*************************************************/
if(get_theme_mod('bevesi_paginate_type') == 'loadmore' || get_theme_mod('bevesi_paginate_type') == 'infinite' || bevesi_ft() == 'load-more' || bevesi_ft() == 'infinite'){
	if(get_theme_mod('bevesi_ajax_on_shop',0) == '1'){
		function bevesi_load_more_scripts() {
			if(is_shop() || is_tax()){
				if(get_theme_mod('bevesi_paginate_type') == 'infinite' || bevesi_ft() == 'infinite'){
					wp_enqueue_script( 'bevesi-load-more',  plugins_url( 'js/infinite-scroll.js', __FILE__ ), true );
				} elseif(get_theme_mod('bevesi_paginate_type') == 'loadmore' || bevesi_ft() == 'load-more') {
					wp_enqueue_script( 'bevesi-load-more',  plugins_url( 'js/load_more.js', __FILE__ ), true );
				}
			}
		}
		...
		function bevesi_load_more_scripts() {
			if(is_shop() || is_tax()){
				...
				wp_localize_script( 'bevesi-load-more', 'loadmore', array(
					...
					'no_more_products' => esc_html__('No More Products','bevesi-core'),
	...
	add_action( 'wp_enqueue_scripts', 'bevesi_load_more_scripts' );
}
...
add_action( 'wp_ajax_nopriv_load_more', 'bevesi_load_more_callback' );
add_action( 'wp_ajax_load_more', 'bevesi_load_more_callback' );
function bevesi_load_more_callback() {
